...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	return `${(($$result.head += `<!-- HEAD_svelte-ui484d_START -->${(($$result.title = `<title>Ringe</title>`), '')}<meta name="description" content="Rahoff Ringe"><!-- HEAD_svelte-ui484d_END -->`), '')} ${validate_component(
		Container,
		'Container'
	).$$render(
		$$result,
		...
		{},
		{
			default: () => {
				...
				)} <h1 data-svelte-h="svelte-17f6pjv">Ringe</h1> ${validate_component(
					ElementGrid,
					'ElementGrid'
				).$$render(
					$$result,
